...
export class Map {
    private static worldImgContainer = document.getElementById("map_world_img_container");
    private static localIMGElement = <HTMLImageElement>document.getElementById("local_img");
    private static mapIconListElement = <HTMLElement>document.getElementById("map_icon_list");
    private static fontProperties: MapFontProperties = {
        city: 32,
        placeName: 24,
    };
    private static iconListVisible: boolean = false;
    private static playerMarker: MapPlayerMarker = { tagType: "dot", visible: false, x: 0, y: 0 };
    ...
    private static findLocalMapTags(variable: string, type: string): Object | boolean {
        if (type === "character") {
            let objects = GamePieces.characters.filter(
                (object) => object.type === "character" && object.src.indexOf(variable) !== -1
            );
            if (objects.length > 0) {
