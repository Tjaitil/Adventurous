...
export class Map {
  ...
  private static loadLocalMapTags() {
    this.localMapTags.forEach(element => {
      const pos = this.findLocalMapTags(element.tagIdentifier, 'character');
      if (pos === false) {
  ...
  private static findLocalMapTags(
    ...
    if (type === 'character') {
      const objects = GamePieces.characters.filter(
        object =>
          object.type === 'character' && object.src.indexOf(variable) !== -1,
      );
      if (objects.length > 0) {
  ...
  public static toggleMapType() {
    if (this.mapType === 'local') {
      ...
      document.getElementById('map_type_toggle_overlay').style.visibility =
        'visible';
      this.mapType = 'world';
      document
        .getElementById('map_container_header')
        .querySelectorAll('h2')[0].innerText = jsUcfirst(this.mapType) + ' map';
    } else {
