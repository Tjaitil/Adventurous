...
export class conversation {
    ...
    public static setup() {
        ...
        this.conversationHeader = document.getElementById(
            'conversation-header',
        );
    ...
    public static async loadConversation(character) {
        ...
        await this.getNextSegment(data);
    }
    ...
    private static async getNextSegment(
        data: ConversationRequest,
    ): Promise<void> {
        CustomFetchApi.post<conversationSegmentResponse>(
            'conversation/next',
            data,
        )
            .then(response => {
    ...
    private static async handleUserEvent(event: MouseEvent) {
        ...
        if (eventTargetElement.tagName === 'BUTTON') {
            text =
                this.conversationTextWrapper.querySelectorAll('li')[0]
                    .innerText;
        } else {
        ...
        const data = {
            person: this.currrentPerson,
            is_starting: false,
            selected_option: this.selectedConversationOption.id,
        };

        this.getNextSegment(data);
    }
