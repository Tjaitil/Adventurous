...
addEventListener("load", function () {
    conversation.conversationDiv = document.getElementById("conversation-text-wrapper").querySelectorAll("ul")[0];
    document
        .getElementById("conversation-container")
        .querySelectorAll("img")[0]
        .addEventListener("click", () => conversation.endConversation());
    /*conversation.toggleButton();*/
    conversation.button = <HTMLButtonElement>document.getElementById("conv_button");
...
export class conversation {
    ...
    public static setup() {
        ...
        this.conversationTextWrapper = document.getElementById("conversation-text-wrapper");
    }
    ...
    public static async loadConversation(character) {
        ...
        if (Game.properties.device == "mobile") {
            ...
            if (conversationContainerHeight > 170) {
                ...
                this.conversationContainer.style.height = conversationContainerHeight + "px";
            }
        }
    ...
    private static handleToggleButton() {
        if (this.buttonToggle === false) {
            ...
            let text = this.currentConversationSegment.header ??= "Select an answer";
    ...
    private static handleCallbacks() {
        ...
        switch (this.selectedConversationOption.callback) {
            ...
            case "gameTravel.travel":
                triggerEvent = () => {
                    gameTravel.travel(this.selectedConversationOption.option_value, this.currrentPerson);
                }
                break;
            default:
                break;
    ...
    private static async getNextSegment(url: string): Promise<void> {
        CustomFetchApi.get<conversationSegmentResponse>(url).then((response) => {
    ...
    private static async handleUserEvent(event: MouseEvent) {
        ...
        this.selectedConversationOption =
            this.currentConversationSegment.options.find((segment) => segment.text === text);
    ...
    private static togglePerson() {
        ...
        if (this.selectedConversationOption.container === "A" ||
            this.selectedConversationOption.person !== "player"
        ) {
            this.personContainerA.style.visibility = "visible";
            this.personContainerA.src = AssetPaths.getImagePath(this.selectedConversationOption.person + ".png");
            this.conversationHeader.innerText = jsUcWords(this.selectedConversationOption.person);
            this.personContainerB.style.visibility = "hidden";
        } else {
            this.personContainerB.src = AssetPaths.getImagePath("character image.png");
    ...
    private static resetEventButton() {
        let clone = <HTMLButtonElement>this.button.cloneNode(true);
    ...
    private static makeLinks() {
        ...
        if (this.currentConversationSegment.options.length > 1) {
            ...
            this.selectedConversationOption = this.currentConversationSegment.options[0];
            let li = document.createElement("li");
