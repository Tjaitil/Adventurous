...
const adventuresModule = { 
    init() {
        if(document.getElementById("provide") != null && document.getElementById("provide").children.length > 0) {
            ...
            [...document.getElementsByClassName("adventure-provide-button")].forEach(element =>
        ...
        if(document.getElementById("invite")) {
            let buttons = document.getElementById("invite").querySelectorAll("button");
            ...
            document.getElementById("hire-citizen").addEventListener("click", () => this.hireCitizen());
        }
        // Add event to the warriors
        this.addWarriorEvents();
        
        document.getElementById("adventure-start-event").addEventListener("click", () => this.newAdventure());
        document.getElementById("adventure-get-report-event").addEventListener("click", () => this.newAdventure());
    },
    intervalID: null,
    getCountdown() {
            ...
            if(response[0] != false) {
                ...
                this.intervalID = setInterval(() => {
                    ...
                    if (remainder < 0 && status === "1"){
                        ...
                        document.getElementById("adventure-get-report-event").style.display = "block";
                        document.getElementById("time").innerHTML = "Finished";
                        document.getElementById("adventure-leave-event").style.display = "none";
                    }
                    else if (remainder < 0) {
                        clearInterval(this.intervalID);
                        document.getElementById("adventure-get-report-event").style.display = "none";
    ...
    newAdventure() {
        ...
        if(!form.reportValidity()) {
            gameLogger.addMessage("Please fill out all required inputs in the form", true);
            return false;
        }
    ...
    joinAdventure(id) {
        if(!id) return false;
        let data = "model=AdventureRequest" + "&method=joinAdventure" + "&id=" + id;
        ajaxP(data, function(response) {
    ...
    checkUser() {
        ...
        let data = "model=Adventures" + "&method=checkUser" + "&username=" + input;
        ajaxG(data, response => {
            if(response[0] != false) {
                if(response[1] == "") {
    ...
    updateAdventure() {
        ...
        ajaxP(data, response => {
            if(response[0] !== false) {
                ...
                document.getElementById("adventure-navigate-back").addEventListener("click", 
                    () => inputHandler.fetchBuilding('adventures')
                );
                updateInventory();
            }
