...
const adventuresModule = {
  init() {
    if (
      ...
      [...document.getElementsByClassName('adventure-provide-button')].forEach(
        element => element.addEventListener('click', () => this.provide()),
      );
      ...
      if (response[0] != false) {
        ...
        this.intervalID = setInterval(() => {
          ...
          if (remainder < 0 && status === '1') {
            ...
            document.getElementById('adventure-leave-event').style.display =
              'none';
          } else if (remainder < 0) {
            clearInterval(this.intervalID);
            document.getElementById(
  ...
  newAdventure() {
    ...
    let data =
      'model=SetAdventure' + '&method=newAdventure' + '&JSON_data=' + JSON_data;
    ...
    ajaxP(data, response => {
      if (response[0] !== false) {
        ...
        document
          ...
          .getElementsByTagName('tbody')[0].innerHTML = responseText.html[0];
