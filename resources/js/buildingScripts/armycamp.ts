...
const armycampModule = {
  ...
  checkWarriorAmountForAction() {
    ...
    const warning = document.getElementById('multiple-warrior-action-warning');
  ...
  runWarriorAction() {
    const actionName =
      this.actionTypeInput.options[this.actionTypeInput.selectedIndex].value;
    ...
    if (actionName === WarriorActions.HEAL_WARRIOR) {
      ...
      AdvApi.post<RunSingleWarriorActionResponse>(url + actionUrl, data).then(
        response => {
          this.updatePage(actionName, [response.data.warrior] ?? []);
          Inventory.update();
        },
      ...
      if (actionName === WarriorActions.START_TRAINING) {
        ...
        const selectedType =
          this.trainingTypeInput.options[this.trainingTypeInput.selected];
...
class Warrior {
  ...
  private endTraining() {
    ...
    AdvApi.post<RunSingleWarriorActionResponse>('/armycamp/endTraining', data)
      .then(response => {
        this.setData(response.data.warrior);
        this.runCountdown();
      })
