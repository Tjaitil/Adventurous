...
const traderModule = {
  ...
  getTraderAssignmentCountdown() {
    ...
    AdvApi.get<AssignmentCountdownResponse>('/')
      .then(response => {
        const endTime = (response.data.traderAssigmentCountdown + 14400) * 1000;
        const x = setInterval(function () {
          const { remainder, hours, minutes, seconds } =
            countdown.calculate(endTime);
          if (
            document.getElementById('trader_assignments_countdown_time') == null
          ) {
            clearInterval(x);
          } else if (remainder < 1) {
            document.getElementById(
              'trader_assignments_countdown_time',
            ).innerHTML = 'Re enter building to get new trader assignments';
            clearInterval(x);
          } else {
            document.getElementById(
              'trader_assignments_countdown_time',
  ...
  pickUp() {
    ...
    AdvApi.post<AssignmentPickupResponse>('/trader/assignment/update', data)
      .then(response => {
        ...
        // updateCountdownTab();
        document.getElementById('traderAssignment-cart-amount').innerHTML =
          '' + response.data.cartAmount;
      })
      .catch(() => false);
  },
  deliver() {
