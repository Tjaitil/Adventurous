...
const handleUserEvent = (optionId: number | null = null) => {
  if (currentConversationSegment.value == null || store.currentPerson == null) {
    return;
  }
  const selectedId = optionId ?? currentConversationSegment.value.options[0].id;
  ...
  switch (selectedConversationOption.value.client_callback) {
    case 'GameTravelCallback':
      ...
      if (
        ...
        typeof selectedConversationOption.value.option_values['location'] !==
          'string'
      ) {
        GameLogger.addMessage('An error occured', true);
        return;
...
const getNextSegment = async (data: ConversationRequest): Promise<void> => {
  await CustomFetchApi.post<conversationSegmentResponse>(
    ...
    .then(response => {
      currentConversationSegment.value = response.data.conversation_segment;
