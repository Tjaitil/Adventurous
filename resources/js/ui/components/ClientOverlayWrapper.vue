...
const vuePageMap: Record<
  VuePage,
  { component: VuePageComponent; preload: () => Promise<void> }
> = {
  armory: {
    component: ArmoryPage,
    preload: () => buildingDataPreloader.preloadArmory(),
  },
  smithy: {
    component: SmithyPage,
    preload: () => buildingDataPreloader.preloadSmithy(),
  },
  bakery: {
    component: BakeryPage,
    preload: () => buildingDataPreloader.preloadBakery(),
  },
  archeryshop: {
    component: ArcheryShopPage,
    preload: () => buildingDataPreloader.preloadArcheryShop(),
  },
  travelbureau: {
    component: TravelBureauPage,
    preload: () => buildingDataPreloader.preloadTravelBureau(),
  },
  zinsstore: {
    component: ZinsStorePage,
    preload: () => buildingDataPreloader.preloadZinsStore(),
  },
};
...
const unsubRenderBuilding = gameEventBus.subscribe('RENDER_BUILDING', obj => {
  ...
  if (!('content' in obj) && !('loading' in obj)) {
    const entry = vuePageMap[obj.building];
    if (entry) {
