...
export class BaseAxios {
    ...
    protected static fetchInstance<T extends {}>(method: 'PUT' | 'GET' | 'POST', url: string, data?: Object): Promise<T> {
    ...
    public static async baseGet<T = advAPIResponse>(url: string): Promise<T> {
        ...
        return await this.Axios.post<T>('GET', url).then(async (response) => response.data);

    }

    public static async basePost<T = advAPIResponse>(url: string, data: Object): Promise<T> {
        if(!this.Axios) this.createAxiosInstance();
        
        return await this.Axios.post<T>('POST', url, data).then(async (response) => response.data);
    }
