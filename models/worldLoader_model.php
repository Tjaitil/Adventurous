...
class worldLoader_model extends model {
                ...
                if(in_array($string['layers'][$i]['name'], array("Objects", "Buildings", "Characters", "Figures")) === true) {
                    ...
                    for($x = 0; $x < count($string['layers'][$i]['objects']); $x++) {
                        ...
                        if($string['layers'][$i]['name'] === "Buildings") {
                            switch ($object_array[$x]['src']) {
                                case 'city centre.png':
                                    $object_array[$x]['src'] = "citycentre.png";
                                    break;
                                case 'army camp.png':
                                    $object_array[$x]['src'] = "armycamp.png";
                                    break;
                                default:

                                    break;
                            }
                            if(!isset($object_array[$x]['displayName'])) {
                                $object_array[$x]['displayName'] = trim(substr($object_array[$x]['src'], 0, -4));
                            }
                            if($this->map === "9.9") {
                        ...
                        if($object_array[$x]['rotation'] !== 0) {
                            switch($object_array[$x]['rotation']) {
                                case 90:
                                ...
                                case 180:
                                    $width = $object_array[$x]['width'];
                                    $object_array[$x]['x'] = $object_array[$x]['x'] - $object_array[$x]['width'];
                                    $object_array[$x]['y'] = $object_array[$x]['y'] + $object_array[$x]['height'];
                                    break;
                                default:
                        ...
                        if($object_array[$x]['type'] == 'character') {
                            if(in_array($object_array[$x]['src'], array('Woman character.png', 'Character13.png', 
                                                                        'Citizen.png'))) {
                                $object_array[$x]['conversation'] = false;
                            }
                            else {
                                $object_array[$x]['conversation'] = true;
                            }
                            $object_array[$x]['displayName'] = $this->setDisplayName($object_array[$x]['type']);
                        }
        ...
        private function setDisplayName($src) {
            // Set display names;
            $display_name = "";
            switch($src) {
                ...
                case 'tutorial_sailor.png':
                    ...
                    break;
                default: 
                    $display_name = substr($object_array[$x]['src'], 0, -4);
                    break;
            }
            return $display_name;
        }
