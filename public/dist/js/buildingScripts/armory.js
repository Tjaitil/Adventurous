...
const armoryModule = {
    ...
    addClickEvents(childIndex) {
        ...
        elements.forEach(element => element.addEventListener("click", event => this.removeArmor(event)));
    },
    toggleOption() {
    ...
    wearArmor() {
        let selectElement = document.getElementById("select_warrior");
        ...
        if (ItemSelector.isItemValid() === false)
            return false;
        ...
        AdvApi.post('/api/armory/add', data).then((response) => {
    ...
    removeArmor(event) {
        ...
        if (!parent.querySelectorAll(".armory_view_warrior_id")[0])
            return false;
        ...
        AdvApi.post('/api/armory/remove', data).then((response) => {
            document.getElementById("selected").innerHTML = "";
            this.replaceWarriorContainer(response.html.warrior_armory, data.warrior_id);
            Inventory.update();
        }).catch(() => false);
    },
    replaceWarriorContainer(newContainer, index) {
        ...
        if (!warriorContainer[replaceIndex])
            return false;
