...
export class Daqloon {
    ...
    hit(direction) {
        ...
        viewport.drawText("18px Times New Roman", "#FFFFFF", damage, this.drawX - GamePieces.player.xMovement * viewport.scale + 40, this.drawY - GamePieces.player.yMovement * viewport.scale + 20);
        this.drawHealthBar(this.drawX - GamePieces.player.xMovement * viewport.scale, this.drawY - GamePieces.player.yMovement * viewport.scale);
        if (this.health <= 0) {
            this.spriteXIndex = 0;
            this.dead = true;
        }
    ...
    locater() {
        ...
        if (this.fighting_area.diameterUp > GamePieces.player.ypos ||
    ...
    drawOnCanvas() {
        viewport.drawSprite(this.sprite, this.spriteXIndex * 32, this.spriteYIndex * 32, 32 * viewport.scale, 32 * viewport.scale, this.drawX - GamePieces.player.xMovement, this.drawY - GamePieces.player.yMovement, this.width, this.height);
    ...
    draw() {
        ...
        if (((Math.abs(this.diameterLeft - GamePieces.player.xpos) < 20 ||
            Math.abs(this.diameterRight - GamePieces.player.xpos) < 20) && Math.abs(this.diameterUp + (this.height / 2) - GamePieces.player.diameterUp) < 20)
            &&
                this.attack === false &&
            this.cooldown === false &&
            this.spawn === false) {
            ...
            if (GamePieces.player.combatActions.block) {
                console.log("BLOCKED");
                viewport.drawText("18px Times New Roman", "#FFFFFF", "BLOCKED", 20, viewport.height / 2);
            }
            else {
        ...
        else if (Game.properties.duration % 6 === 0 &&
            this.attack === true &&
            this.dead === false &&
            this.spawn === false) {
        ...
        else if (Game.properties.duration % 10 === 0 &&
            this.dead === false &&
            this.spawn === false &&
            this.attack === false) {
            this.spriteXIndex++;
