...
export class Item {
    ...
    draw() {
        if (this.loopIndex > 3)
            this.loopIndex = 0;
        viewport.drawSprite(this.spriteObject.image, 0, 0, 32 * viewport.scale, 32 * viewport.scale, this.drawX - GamePieces.player.xMovement * viewport.scale + 5, this.drawY - GamePieces.player.yMovement * viewport.scale + this.loopArray[this.loopIndex], this.width * this.scale, this.height * this.scale);
        this.shadow.draw();
    ...
    pickUpItem() {
        if (Math.abs(this.x - GamePieces.player.xpos) <= 10 && Math.abs(this.y - GamePieces.player.ypos) <= 10) {
            ...
            GamePieces.items = GamePieces.items.filter((item) => {
...
export class ItemShadowAnimation {
    ...
    draw() {
        if (this.indexX > 3)
            this.indexX = 0;
        viewport.drawSprite(this.spriteObject.image, this.loopArray[this.indexX] * 32, 0, 32 * viewport.scale, 32 * viewport.scale, this.drawX - GamePieces.player.xMovement * viewport.scale - 8, this.drawY - GamePieces.player.yMovement * viewport.scale - 8, 48, 48);
        if (Game.properties.duration % 20 === 0) {
            this.indexX++;
        }
    }
