...
window.addEventListener("load", () => {
    let wrappers = <HTMLElement[]>[...document.getElementsByClassName("profiency-level-wrapper")];
    console.log(wrappers);
    wrappers.forEach((element) =>
        element.addEventListener("click", () => getSkillXp(element, element.dataset.wrapperSkill))
    );
});

export async function getSkillXp(element: HTMLElement, skill: string) {
    ...
    let tooltip = <HTMLElement>element.querySelectorAll(".skill_tooltip")[0];
