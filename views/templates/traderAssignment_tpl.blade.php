...
<div id="traderAssignment_current" class="mb-2">
    @component('components.BorderInterfaceContainer')
        <p class="mb-0 col-span-full">Current trader assignment details</p>
        @if (isset($Trader->traderAssignment->id))
            ...
            @php
                $hasCapasity = $Trader->cart_amount < $Trader->cartData->capasity && $Trader->cart_amount + $Trader->delivered < $CurrentAssignment->assignment_amount;
            @endphp
            <div id="traderAssignment-cart-amount-wrapper">
                Cart Capasity<br>
                <span id="traderAssignment-cart-amount">{{ $Trader->cart_amount }}</span> /
                <span>{{ $Trader->cartData->capasity }}</span>
            </div>
